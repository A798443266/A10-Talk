...
public class PickContactAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // 显示数据
        holder.tv_name.setText(pickContactInfo.getContact().getName());
        holder.cb.setChecked(pickContactInfo.isChecked());
        Glide.with(mContext).load(pickContactInfo.getContact().getPic()).error(R.drawable.logo).into(holder.cv_user_pic);

        // 判断(如果群中已经存在的成员，要选择且不可以取消)
        if(mExistMembers.contains(pickContactInfo.getContact().getPhone())) {
    ...
    public List<String> getPickContacts() {
        ...
        for (PickContactInfo pick: mPicks){
            // 判断是否选中
            if(pick.isChecked()) {
                picks.add(pick.getContact().getPhone());
            }
        }
        return picks;
    }
