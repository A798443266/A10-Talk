...
public class InviteTableDao {
    ...
    public void addInvitation(InvationInfo invitationInfo) {
        ...
        ContactInfo contact = invitationInfo.getContact();

        if (contact != null) {// 联系人
            values.put(InviteTable.COL_USER_HXID, invitationInfo.getContact().getPhone());
            values.put(InviteTable.COL_USER_NAME, invitationInfo.getContact().getName());
            values.put(InviteTable.COL_USER_PIC, invitationInfo.getContact().getPic());
        } else {// 群组
    ...
    public List<InvationInfo> getInvitations() {
        ...
        while (cursor.moveToNext()) {
            ...
            if (groupId == null) {// 联系人的邀请信息
                ...
                contactInfo.setPhone(cursor.getString(cursor.getColumnIndex(InviteTable.COL_USER_HXID)));
                contactInfo.setName(cursor.getString(cursor.getColumnIndex(InviteTable.COL_USER_NAME)));
                contactInfo.setPic(cursor.getString(cursor.getColumnIndex(InviteTable.COL_USER_PIC)));
                //添加联系人信息进邀请信息
                invationInfo.setContact(contactInfo);
            } else {// 群组的邀请信息
