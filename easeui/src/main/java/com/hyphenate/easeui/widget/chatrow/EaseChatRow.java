...
import com.hyphenate.easeui.utils.GlideGetCircle;
    ...
    private void setUpBaseView() {
        ...
        if (userAvatarView != null) {
            ...
            if (message.direct() == Direct.SEND) {
                ...
                //发消息显示自己的头像昵称  （以后从本地读取头像显示，如果本地没有再去请求服务器）
                //得到本地sp存放的头像地址
                String a = context.getSharedPreferences("SP_EIPLAN", Context.MODE_PRIVATE).getString("current_pic", "");
//                Toast.makeText(context,a,Toast.LENGTH_SHORT).show();
                Glide.with(context).load(a).error(R.drawable.logo).transform(new GlideGetCircle(context)).into(userAvatarView);
//                EaseUserUtils.setUserAvatar(context, EMClient.getInstance().getCurrentUser(), userAvatarView);
            } else {//接收到消息设置别人的头像
                //message.getFrom()就是发消息者的环信ID
                String url = message.getStringAttribute("picturl", "");//收到发送人的头像地址
//                Toast.makeText(context,url,Toast.LENGTH_SHORT).show();
                Glide.with(context).load(url).error(R.drawable.logo).transform(new GlideGetCircle(context)).into(userAvatarView);
